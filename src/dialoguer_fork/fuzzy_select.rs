use super::theme::{SimpleTheme, TermThemeRenderer, Theme};
...
impl FuzzySelect<'_> {
    ...
    #[inline]
    pub fn interact(&self) -> io::Result<(bool, usize)> {
    ...
    #[inline]
    pub fn interact_opt(&self) -> io::Result<Option<(bool, usize)>> {
        self.interact_on_opt(&Term::stderr())
    }

    /// Like `interact` but allows a specific terminal to be set.
    #[inline]
    pub fn interact_on(&self, term: &Term) -> io::Result<(bool, usize)> {
    ...
    /// Like `interact` but allows a specific terminal to be set.
    #[inline]
    pub fn interact_on_opt(&self, term: &Term) -> io::Result<Option<(bool, usize)>> {
        self._interact_on(term, true)
    }

    /// Like `interact` but allows a specific terminal to be set.
    fn _interact_on(&self, term: &Term, allow_quit: bool) -> io::Result<Option<(bool, usize)>> {
        ...
        loop {
            ...
            match (term.read_key()?, sel) {
                ...
                (Key::End, _) => {
                    position = search_term.len();
                    term.flush()?;
                }
                (key @ (Key::Enter | Key::Char(' ')), Some(sel)) if !filtered_list.is_empty() => {
                    if self.clear {
                        render.clear()?;
                    }

                    let is_enter = key == Key::Enter;

                    if self.report && !is_enter {
                        render
                    ...
                    term.show_cursor()?;
                    match sel_string_pos_in_items {
                        Some(x) => return Ok(Some((is_enter, x))),
                        None => return Ok(None),
                    }
                }
