use std::{cmp::min, cmp::Ordering, fmt::Debug, io, iter, str::FromStr};
        ...
        loop {
            ...
            loop {
                match term.read_key()? {
                    ...
                    Key::Home => {
                        let diff = (position + prompt_len) % term.size().1 as usize;
                        if diff == 0 {
                            // Do nothing, already at the start of the line
                        } else {
                            let left_offset = min(diff, position);
                            term.move_cursor_left(left_offset)?;
                            position -= left_offset;
                        }
                        term.flush()?;
                    }
                    Key::End => {
                        let diff = (position + prompt_len) % (term.size().1 as usize - 1);
                        if diff == 0 {
                            // Do nothing, already at the end of the line
                        } else {
                            let right_offset =
                                min(chars.len() - position, term.size().1 as usize - 1 - diff);
                            term.move_cursor_right(right_offset)?;
